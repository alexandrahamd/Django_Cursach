...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        articals = [
            {'title': 'Новости', 'content': 'Свежие новости на сегодня', },
            {'title': 'Рассылки', 'content': 'Актуальные Рассылки', },
            {'title': 'Советы', 'content': 'Советы оформления рассылок', },
            {'title': 'Реклама', 'content': 'Как рассылать рекламу', },
            {'title': 'Спам', 'content': 'Как нельзя отправлять рассылки', },
        ]

        articals_list = []
        for item in articals:
            articals_list.append(Blog(**item))

        Blog.objects.bulk_create(articals_list)
